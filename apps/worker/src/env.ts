...
export function readEnv(): WorkerEnv {
  return {
    ...
    graphPollIntervalSeconds: readNumber(process.env.FIBER_GRAPH_POLL_INTERVAL_SECONDS, 60),
    ckbRpcUrl: process.env.CKB_RPC_URL,
    ckbExplorerBaseUrl: process.env.CKB_EXPLORER_BASE_URL,
    ckbEnrichIntervalSeconds: readNumber(process.env.CKB_ENRICH_INTERVAL_SECONDS, 180),
    enableReachabilityProbes: process.env.FIBERSCOPE_ENABLE_REACHABILITY_PROBES === "true",
    reachabilityProbeIntervalSeconds: readNumber(
      process.env.FIBERSCOPE_REACHABILITY_PROBE_INTERVAL_SECONDS,
      300,
    ),
    reachabilityTimeoutMs: readNumber(process.env.FIBERSCOPE_REACHABILITY_TIMEOUT_MS, 2_500),
    reachabilityProbeLimit: readNumber(process.env.FIBERSCOPE_REACHABILITY_PROBE_LIMIT, 50),
