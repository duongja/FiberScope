...
const fallbackAssetColors = ["#3a7d9f", "#6c7a89", "#8a6fb8", "#2f7d68", "#a26140"];
const mapWidth = 1280;
const mapHeight = 620;
...
export function NetworkMap({
  ...
  const [channelLimit, setChannelLimit] = useState(defaultChannelLimit(channels.length));
  ...
  const options = useMemo(() => availableChannelLimitOptions(channels.length), [channels.length]);
    ...
    <section className="network-map-panel">
      <div className="section-head">
        <div>
          <h2>Network map</h2>
          <p>
            {selectedChannels.length} selected channels collapsed into {topology.visibleEdges.length} node-to-node links.
          </p>
        </div>
        <div className="map-actions">
          <label className="map-control">
            <span>Channels</span>
            <select value={channelLimit} onChange={(event) => setChannelLimit(event.target.value)}>
              {options.map((option) => (
                <option key={option.value} value={option.value}>{option.label}</option>
              ))}
            </select>
          </label>
          <Link className="button secondary" href="/channels">Open channels</Link>
        </div>
      </div>

      <div className="network-map-canvas">
        <div className="channel-map-readout">
          <span><strong>{selectedChannels.length}</strong> selected channels</span>
          <span className="readout-arrow">become</span>
          <span><strong>{topology.visibleEdges.length}</strong> drawn links</span>
        ...
        >
          <rect x="0" y="0" width={mapWidth} height={mapHeight} rx="8" fill="#ffffff" />
          <g opacity="0.62">
            <circle cx="640" cy="300" r="235" fill="none" stroke="#e3ebf3" strokeWidth="1" />
            <circle cx="640" cy="300" r="104" fill="none" stroke="#e8f4ef" strokeWidth="1" />
            <line x1="96" y1="300" x2="1184" y2="300" stroke="#edf1f5" strokeWidth="1" />
            <line x1="640" y1="64" x2="640" y2="556" stroke="#edf1f5" strokeWidth="1" />
          </g>
          ...
          <g>
            {topology.visibleEdges.map((edge) => {
              ...
              const opacity = activeCount > 0 ? 0.76 : 0.28;
              return (
                <path
                  d={edgePath(from, to)}
                  fill="none"
          ...
          <g>
            {topology.visibleEdges.map((edge) => {
              const from = topology.positions.get(edge.from);
              const to = topology.positions.get(edge.to);
              if (!from || !to || !shouldShowEdgeLabel(edge, topology.visibleEdges.length)) {
                return null;
              }
              const label = edgeLabelPoint(from, to);
              return (
                <g className="edge-count-label" key={`${edge.key}:label`} transform={`translate(${label.x} ${label.y})`}>
                  <rect x="-13" y="-10" width="26" height="20" rx="10" />
                  <text textAnchor="middle" y="4">{edge.count}</text>
                </g>
              );
            })}
          </g>
                ...
                <a href={`/nodes/${point.node.pubkey}`} key={point.node.pubkey}>
                  <g className="network-node">
                    <circle
                      cx={point.x}
                      cy={point.y}
                      fill={point.node.stale ? "#8493a3" : nodeColor(point.rankScore)}
                    ...
                    <circle
                      ...
                      opacity="0.9"
                      r={point.radius + 7}
                      stroke={point.node.stale ? "#d8dee5" : "#c7eadb"}
                      strokeWidth="3"
                    />
                    <text
                      fill="#ffffff"
        ...
        <div className="network-map-legend">
          {topology.assetLegend.map((asset, index) => (
            <span className="legend-item" key={asset.symbol}>
              <span className="legend-swatch" style={{ background: assetColor(asset.symbol, index) }} />
              {asset.symbol} <small>{asset.count}</small>
            </span>
          ))}
          <span className="legend-note">{topology.visibleNodes.length} nodes · {topology.visibleEdges.length} links · {topology.visibleChannelCount} mapped channels</span>
        </div>
      </div>
    </section>
  );
...
function buildTopology(nodes: NodeLike[], selectedChannels: ChannelLike[]) {
  ...
  for (const channel of selectedChannels) {
    if (!nodeByPubkey.has(channel.node1Pubkey) || !nodeByPubkey.has(channel.node2Pubkey)) {
      continue;
    }

    const enabledDirections = channel.directions.filter((direction) => direction.enabled).length;
  ...
  const maxChannelCount = Math.max(...Array.from(nodeStats.values()).map((stats) => stats.channelCount), 1);
  const rankedNodes = nodes
    .map((node) => {
      const stats = nodeStats.get(node.pubkey) ?? { channelCount: 0, enabledDirections: 0, assetSymbols: new Set<string>() };
      return {
        node,
        stats,
    ...
    .sort((left, right) => right.rankScore - left.rankScore || right.stats.channelCount - left.stats.channelCount || left.node.nodeName.localeCompare(right.node.nodeName));

  const connectedNodes = rankedNodes.filter((entry) => entry.stats.channelCount > 0);
  const selectedNodes = (connectedNodes.length ? connectedNodes : rankedNodes).slice(0, 60);
  const visiblePubkeys = new Set(selectedNodes.map((entry) => entry.node.pubkey));
  const positionedNodes = positionNodes(selectedNodes, maxChannelCount);
  const positions = new Map(positionedNodes.map((point) => [point.node.pubkey, point]));
  const visibleEdges = Array.from(edgeGroups.values())
    .filter((edge) => visiblePubkeys.has(edge.from) && visiblePubkeys.has(edge.to))
  ...
  return {
    ...
    positions,
    visibleEdges,
    visibleChannelCount: visibleEdges.reduce((total, edge) => total + edge.count, 0),
...
function positionNodes(
  ...
  if (nodeCount <= 8) {
    return rankedNodes.map((entry, index) => {
      const angle = (Math.PI * 2 * index) / Math.max(nodeCount, 1) - Math.PI / 2;
      return {
        ...entry,
  ...
  const ringConfigs = [
    { count: Math.min(10, nodeCount), radiusX: 510, radiusY: 238, offset: -Math.PI / 2 },
    { count: Math.min(16, Math.max(nodeCount - 10, 0)), radiusX: 365, radiusY: 168, offset: -Math.PI / 2 + Math.PI / 16 },
    { count: Math.max(nodeCount - 26, 0), radiusX: 205, radiusY: 94, offset: -Math.PI / 2 + Math.PI / 10 },
  ...
  if (nodeIndex < nodeCount) {
    rankedNodes.slice(nodeIndex).forEach((entry, index) => {
      const angle = (Math.PI * 2 * index) / Math.max(nodeCount - nodeIndex, 1) - Math.PI / 2;
      positioned.push({
      ...entry,
...
function nodeRank(node: NodeLike, stats: NodeStats, maxChannelCount: number): number {
  ...
  return routing * 0.34 + liquidity * 0.24 + reachability * 0.14 + channelSignal * 0.2 + assetSignal * 0.04 + freshness * 0.04;
}

function nodeRadius(entry: { stats: NodeStats; rankScore: number }, maxChannelCount: number): number {
...
function edgeLabelPoint(from: PositionedNode, to: PositionedNode): { x: number; y: number } {
  const dx = to.x - from.x;
  const dy = to.y - from.y;
  const distance = Math.max(Math.hypot(dx, dy), 1);
  const normalX = -dy / distance;
...
function shouldShowEdgeLabel(edge: EdgeGroup, visibleEdgeCount: number): boolean {
...
function primaryAssetSymbol(assetCounts: Map<string, number>): string {
  return Array.from(assetCounts.entries()).sort((left, right) => right[1] - left[1] || left[0].localeCompare(right[0]))[0]?.[0] ?? "CKB";
}

function assetColor(symbol: string, fallbackIndex = 0): string {
  return assetColors.get(symbol) ?? fallbackAssetColors[fallbackIndex % fallbackAssetColors.length];
}
...
function availableChannelLimitOptions(channelCount: number): Array<{ label: string; value: string }> {
...
function selectChannelsByAsset(channels: ChannelLike[], limitValue: string): ChannelLike[] {
  ...
  const buckets = Array.from(groups.values()).sort((left, right) => right.length - left.length);
  let cursor = 0;
  while (selected.length < limit && buckets.some((bucket) => cursor < bucket.length)) {
